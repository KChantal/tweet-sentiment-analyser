...
class AnalysisResults extends Component{
    ...
    componentDidMount() {
        ...
        var line = d3.line()
                    ...
                    .y(function(d) { return yScale(d.avgScore); }) // set the y values for the line generator 
        ...
        svg.selectAll(".dot")
            ...
            .attr("cy", function(d) { return yScale(d.avgScore) })
            .attr("r", 6)
    ...
    render () {
        ...
        const totalAvg = data.reduce((acc, curr) => {
        ...
        if (totalAvg < 0 && totalAvg > -100) {
            overallSentiment = <p>The overall sentiment in tweets 
                            containing your search term was <br></br><span className="negative">negative</span></p>
        } else if (totalAvg < -101) {
            overallSentiment = <p>The overall sentiment in tweets 
                            containing your search term was <br></br>
                            <span className="negative">extremely negative</span></p>
        }
        else if (totalAvg > 0 && totalAvg < 101) {
            overallSentiment = <p>The overall sentiment in tweets 
                            containing your search term was <br></br>
                            <span className="positive">positive</span></p>
        } else if (totalAvg > 100) {
            overallSentiment = <p>The overall sentiment in tweets 
                            containing your search term was<br></br><span className="positive">extremely positive!</span></p>
        }

        const posWords = JSON.parse(JSON.parse(this.props.words.slice(2)).positiveWords);
        const posLiEls = [];
        for (let i = 0; i < posWords.length; i++) {
            posLiEls.push(<li key={`posWord-${i}`}>{posWords[i]}</li>)
        }

        const negWords = JSON.parse(JSON.parse(this.props.words.slice(2)).negativeWords);
        ...
        return (
            <div className="results-div">
                ...
                <p className="positive"><em>Most common positive words in Tweets: </em></p> 
                <ul className="positive">
                    {posLiEls}
                </ul>

                <p className="negative"><em>Most common negative words in Tweets: </em></p>
